...
public class TradePacketsServer {
    public static void register(){
        ...
        CommonEconomy.packets.registerServerReceiver(
                ...
                (payload, server, player) -> Objects.requireNonNull(server).execute(() -> CommonEconomy.packets.sendToPlayer(player, new TradeSendPlayerBalanceS2CPacket(CommonEconomy.getManager(player.level().getServer()).getBalance(player.getUUID(), false)))));
