...
import com.wanomaniac.economy.util.IdentityCompat;
...
import com.wanomaniac.economy.shop.ShopManager;
import com.wanomaniac.economy.shop.ShopListing;
import com.wanomaniac.economy.shop.ShopUi;
import com.wanomaniac.economy.orders.OrderManager;
import com.wanomaniac.economy.orders.OrderRequest;
import com.wanomaniac.economy.orders.OrdersUi;
...
public final class EconomyCommands {
    ...
    private static int showBalance(IdentityCompat.PlayerRef target, CommandSourceStack source) {
        EconomyManager manager = CommonEconomy.getManager(source.getServer());
        ...
        if (executor != null && executor.getUUID().equals(target.id())) {
            source.sendSuccess(() -> Component.literal("Balance: " + CommonEconomy.formatMoney(bal))
                    .withStyle(ChatFormatting.YELLOW), false);
        } else {
            source.sendSuccess(() -> Component.literal(target.name() + "'s balance: " + CommonEconomy.formatMoney(bal))
                    .withStyle(ChatFormatting.YELLOW), false);
        }
    ...
    private static int pay(ServerPlayer from, String target, long amount, CommandSourceStack source) {
        ...
        if (manager.pay(from.getUUID(), toId, amount)) {
            ...
            source.sendSuccess(() -> Component.literal("Paid " + CommonEconomy.formatMoney(amount) + " to " + displayName)
                    .withStyle(ChatFormatting.GREEN), false);

            if (toOnline != null) {
                toOnline.sendSystemMessage(Component.literal(from.getName().getString() + " sent you " + CommonEconomy.formatMoney(amount))
    ...
    private static LiteralArgumentBuilder<CommandSourceStack> buildAddMoney() {
        return literal("addmoney").requires(OperatorCommandUtils::setCommandSourceStack) // s.hasPermission(2)
    ...
    private static LiteralArgumentBuilder<CommandSourceStack> buildSetMoney() {
        return literal("setmoney").requires(OperatorCommandUtils::setCommandSourceStack)
                .then(argument("targets", GameProfileArgument.gameProfile())
    ...
    private static LiteralArgumentBuilder<CommandSourceStack> buildRemoveMoney() {
        return literal("removemoney").requires(OperatorCommandUtils::setCommandSourceStack)
                .then(argument("targets", GameProfileArgument.gameProfile())
    ...
    private static LiteralArgumentBuilder<CommandSourceStack> buildRemovePlayer() {
        return literal("removeplayer").requires(OperatorCommandUtils::setCommandSourceStack)
                .then(argument("targets", GameProfileArgument.gameProfile())
    ...
    private static int addMoney(Collection<IdentityCompat.PlayerRef> profiles, long amount, CommandSourceStack source) {
        ...
        if (profiles.size() == 1) {
            ...
            source.sendSuccess(() -> Component.literal(
                            "Added " + CommonEconomy.formatMoney(amount) + " to " + p.name() + "'s balance.")
        ...
        source.sendSuccess(() -> Component.literal(
                        "Added " + CommonEconomy.formatMoney(amount) + " to " + count + " player" + (count > 1 ? "s" : ""))
                .withStyle(ChatFormatting.GREEN), true);
        return count;
    }
    ...
    private static int setMoney(Collection<IdentityCompat.PlayerRef> profiles, long amount, CommandSourceStack source) {
        ...
        if (profiles.size() == 1) {
            ...
            source.sendSuccess(() -> Component.literal(
                            "Set balance of " + p.name() + " to " + CommonEconomy.formatMoney(amount))
                    .withStyle(ChatFormatting.GREEN), true);
            return 1;
        }
        ...
        int count = profiles.size();
        source.sendSuccess(() -> Component.literal(
                        "Set balance to " + CommonEconomy.formatMoney(amount) + " for " + count + " player" + (count > 1 ? "s" : ""))
                .withStyle(ChatFormatting.GREEN), true);
        return count;
    }
    ...
    private static int removeMoney(Collection<IdentityCompat.PlayerRef> profiles, Long amount, CommandSourceStack source) {
        ...
        if (profiles.size() == 1) {
            ...
            if (!manager.removeMoney(id, amount)) {
                source.sendFailure(Component.literal(
                                "Failed to remove " + CommonEconomy.formatMoney(amount) + " from " + p.name() + "'s balance due to insufficient funds.")
                        .withStyle(ChatFormatting.RED));
                return 1;
            }
            source.sendSuccess(() -> Component.literal(
                            "Successfully removed " + CommonEconomy.formatMoney(amount) + " from " + p.name() + "'s balance.")
                    .withStyle(ChatFormatting.GREEN), true);
            return 1;
        }
        ...
        if (success > 0) {
            ...
            if (amount == null) {
                ...
                source.sendSuccess(() -> Component.literal(
                                "Successfully removed " + CommonEconomy.formatMoney(amount) + " from " + finalSuccess + " player" + (finalSuccess > 1 ? "s" : "") + ".")
                        .withStyle(ChatFormatting.GREEN), true);
            }
        }
    ...
    private static LiteralArgumentBuilder<CommandSourceStack> buildToggleScoreboard() {
        return literal("toggleScoreboard").requires(OperatorCommandUtils::setCommandSourceStack)
                .executes(ctx -> toggleScoreboard(ctx.getSource()));
    }

    private static int toggleScoreboard(CommandSourceStack source) {
        boolean enabled = CommonEconomy.getManager(source.getServer()).toggleScoreboard();
    ...
    private static int openShop(ServerPlayer player, CommandSourceStack source) {
        ShopUi.open(player, CommonEconomy.getManager(source.getServer()).getShop());
        return 1;
    }
    ...
    private static int sellItem(ServerPlayer player, long price, CommandSourceStack source) {
        ...
        ShopManager shop = CommonEconomy.getManager(source.getServer()).getShop();
        ...
        source.sendSuccess(() -> Component.literal("Listed item for " + CommonEconomy.formatMoney(price) + " (buyers pay " + CommonEconomy.formatMoney(price + tax) + ")").withStyle(ChatFormatting.GREEN), false);
        return 1;
    }
    ...
    private static LiteralArgumentBuilder<CommandSourceStack> buildOrders() {
        return literal("orders")
                ...
                .then(literal("request")
                        .then(argument("item", AbstractedIdentifierArgument.id())
                                .then(argument("amount", LongArgumentType.longArg(1, EconomyManager.MAX))
                                        .then(argument("price", LongArgumentType.longArg(1, EconomyManager.MAX))
                                                .executes(ctx -> requestItem(ctx.getSource().getPlayerOrException(),
                                                        IdentifierUtils.fromNative(AbstractedIdentifierArgument.getId(ctx, "item")),
    ...
    private static int openOrders(ServerPlayer player, CommandSourceStack source) {
        OrdersUi.open(player, CommonEconomy.getManager(source.getServer()));
        return 1;
    }

    private static int requestItem(ServerPlayer player, ModIdentifier item, int amount, long price, CommandSourceStack source) {
        var holder = net.minecraft.core.registries.BuiltInRegistries.ITEM.getOptional(IdentifierUtils.toNative(item));
        if (holder.isEmpty()) {
            source.sendFailure(Component.literal("Invalid item").withStyle(ChatFormatting.RED));
            return 0;
        }
        OrderManager orders = CommonEconomy.getManager(source.getServer()).getOrders();
        ...
        orders.addRequest(r);
        long tax = Math.round(price * EconomyConfig.get().taxRate);
        source.sendSuccess(() -> Component.literal("Created request (fulfiller receives " + CommonEconomy.formatMoney(price - tax) + ")").withStyle(ChatFormatting.GREEN), false);
        return 1;
    }

    private static int claimOrders(ServerPlayer player, CommandSourceStack source) {
        OrdersUi.openClaims(player, CommonEconomy.getManager(source.getServer()));
        return 1;
    }
    ...
    private static int daily(ServerPlayer player, CommandSourceStack source) {
        ...
        if (manager.claimDaily(player.getUUID())) {
            source.sendSuccess(() -> Component.literal("Claimed " + CommonEconomy.formatMoney(EconomyConfig.get().dailyAmount)).withStyle(ChatFormatting.GREEN), false);
        } else {
    ...
    private static CompletableFuture<Suggestions> suggestPlayers(CommandSourceStack source, SuggestionsBuilder builder) {
        var server = source.getServer();
        var manager = CommonEconomy.getManager(server);
